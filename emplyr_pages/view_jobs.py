...
def display_view_posts(a):
    ...
    for i in range(len(d.keys())):
        ...
        cont.write(f"Application Id : {l[0]}")
        ...
        with col1:
            if st.button(key=f"Delete{l[0]}", label="Withdraw"):
                delete_from_json(l[0])
                cont.write(f"Application ID {l[0]} withdrawn successfully")
